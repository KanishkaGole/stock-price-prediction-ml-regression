...
with tab4:
    st.subheader("Forecast Future Prices (ARIMA Model)")

    df_close = df[['Close']].copy()
    stepwise_model = auto_arima(df_close, start_p=1, start_q=1,
                                 max_p=5, max_q=5, seasonal=False,
                                 d=1, trace=False, error_action='ignore',
                                 suppress_warnings=True, stepwise=True)

    days_ahead = st.slider("Select number of future business days", 1, 365, 30)
    forecast_arima = stepwise_model.predict(n_periods=days_ahead)

    future_dates = pd.date_range(start=df.index[-1] + pd.Timedelta(days=1), periods=days_ahead, freq='B')
    forecast_df = pd.DataFrame({'Date': future_dates, 'Predicted Close': forecast_arima})
    ...
    combined_chart_df = pd.concat([actual_trimmed, forecast_trimmed], axis=0).sort_index()
    ...
    st.download_button("Download Forecast", data=csv_data, file_name="forecast_arima.csv", mime='text/csv')
