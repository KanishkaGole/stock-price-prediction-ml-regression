# filename: app.py or streamlit_app.py
...
def load_data():
    ...
    end_covid = pd.to_datetime("2021-03-31")
    covid_mask = (df['Date'] >= start_covid) & (df['Date'] <= end_covid)

    # Columns to smooth
    smooth_cols = ['Close', 'Open', 'Volume']

    for col in smooth_cols:
        # Create a temporary smoothed series
        smoothed = df.loc[covid_mask, col].rolling(window=5, min_periods=1, center=True).mean()
        # Replace original values in that window with smoothed ones
        df.loc[covid_mask, col] = smoothed
...
# Add descriptions
descriptions = {
    "Linear Regression": "A basic model that fits a straight line. Useful for quick baseline predictions.",
    "Decision Tree": "Splits data using feature thresholds to build a predictive tree. Easy to interpret.",
    "Random Forest": "An ensemble of trees trained on different samples and features for better accuracy.",
    "Support Vector Regressor": "Uses hyperplanes in higher dimensions to predict. Effective for smaller, complex datasets.",
    "AdaBoost Regressor": "Boosts weak learners in sequence by focusing more on previously mispredicted data.",
    "Gradient Boosting": "Sequentially adds trees to fix errors made by previous ones. Good for structured data.",
    "KNN Regressor": "Predicts based on the average of 'k' nearest data points. Simple and effective.",
    "LightGBM Regressor": "A fast gradient boosting model optimized for large datasets with better speed and accuracy."
}

if model_choice:
    st.sidebar.markdown(f"About {model_choice}")
...
with tab1:
    ...
    with st.expander("ℹ️ About the Correlation Heatmap"):
        st.markdown("""
        The heatmap provides a **visual summary of relationships** between numeric features in the dataset. It uses **color gradients** to represent the strength and direction of correlation between pairs of features.
        ...
        #### 🎯 Why it Matters in Stock Prediction
        - Strong correlations between **input features and the target variable (Close)** help machine learning models make accurate predictions.
        - For instance:
        - A high positive correlation between `Open` and `Close` indicates that when the market opens high, it tends to close high too.
        - Volatility features like `STD_5` may show weaker or negative correlation but are still valuable.

        #### 🎨 How to Read the Heatmap
        - **Dark red** (near +1): Strong positive relationship.
        - **Dark blue** (near -1): Strong negative relationship.
        - **White/Light** (near 0): No or very weak linear correlation.
        - Diagonal values are always 1 (a feature is always perfectly correlated with itself).

        Use this heatmap to **identify key drivers**, reduce noise, and guide model input selection.
        """)
...
with tab2:
   # Output results
    ...
    # - **R² Score (Coefficient of Determination):**  Measures how well future samples are likely to be predicted by the model. Higher is better.
